...
latest_prob = None          # float in [0, 1]: probability of bad posture
lock = threading.Lock()
timestamp_ms = 0

# MediaPipe landmark indices (first 17)
NOSE = 0
LEFT_EAR = 7
RIGHT_EAR = 8
LEFT_SHOULDER = 11
...
# Maximum angle (degrees) each check maps to probability = 1.0 
# determined from collect_data.py
MAX_SHOULDER_ANGLE = 15.0   # shoulder line tilt from horizontal
MAX_HEAD_ANGLE = 35.0   # ear line tilt from horizontal
MAX_NECK_ANGLE = 15.0   # nose lateral lean from vertical
...
def _angle_from_horizontal(x1, y1, x2, y2) -> float:
    """Angle in degrees of the line (x1,y1)→(x2,y2) relative to horizontal."""
    return abs(math.degrees(math.atan2(abs(y2 - y1), abs(x2 - x1))))


def bad_posture_probability(landmarks) -> float:
    """
    Returns a float in [0, 1] representing the probability of bad posture,
    computed as a weighted average of three normalized angle deviations:

        prob = Σ(wᵢ · clamp(θᵢ / MAX_θᵢ, 0, 1)) / Σwᵢ

    Weights adjust automatically if a landmark group is not sufficiently visible.
    """
    ...
    # 1. Shoulder tilt – angle of shoulder line from horizontal
    ...
    # 2. Head tilt – angle of ear line from horizontal
    ...
    # 3. Neck lateral lean – angle of nose from vertical through shoulder midpoint
    #    In image coords y increases downward; nose sits above shoulders so
    #    (mid_y - nose_y) is the vertical component and is positive when upright.
    if nose[3] > MIN_VISIBILITY and l_sh[3] > MIN_VISIBILITY and r_sh[3] > MIN_VISIBILITY:
        mid_x = (l_sh[0] + r_sh[0]) / 2
        mid_y = (l_sh[1] + r_sh[1]) / 2
        vert = mid_y - nose[1]         # positive when nose is above shoulders
